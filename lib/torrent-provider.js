...
module.exports = class TorrentProvider {
  ...
  search(query, category, limit, filter) {
    let url = this._getUrl(category, query);
    return url === null
      ? Promise.resolve()
      : this.list(url, limit, filter).then(result => this._postProcess(result));
  }

  last(category, limit, filter) {
    let url = this._getUrl('Last' + (category ? category : ''));
    return url === null
      ? Promise.resolve()
      : this.list(url, limit, filter).then(result => this._postProcess(result));
  }
  ...
  */
  async list(url, limit = null, filter = null) {
    ...
    for (let page = 1; ; page++) {
      ...
      if (limit && res.next) {
        if (
          ...
          (limit.item && limit.item > torrents.length)
        ) {
          url = res.next;
          continue;
        }
    ...
    if (limit && limit.item) torrents = torrents.slice(0, limit.item);
    return torrents;
  }
  ...
  _getMagnet(torrent) {
    return Promise.fromCallback(
      this._x(torrent.desc, 'a[href*="magnet:?xt=urn:btih:"]@href')
    );
  }
